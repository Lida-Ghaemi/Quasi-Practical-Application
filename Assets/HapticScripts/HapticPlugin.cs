...
public class HapticPlugin : MonoBehaviour  {
    ...
    //sinHEffect sinHEffectInstance = new sinHEffect();
    //sinHEffect sinHEffectInstance = obj.AddComponent<sinHEffect>();
    ...
    {
        //Lida*******************************************
        //creating a plane in the position of the ReferencePlane
        /*Ref_Plane = GameObject.Find("ReferencePlane");
        var filter1 = Ref_Plane.GetComponent<MeshFilter>();
        if (filter1 && filter1.mesh.normals.Length > 0)
            normal1 = filter1.transform.TransformDirection(filter1.mesh.normals[0]);

        plane1 = new Plane(normal1, Ref_Plane.transform.position);*/
        //lida//putting markers
        LSLMarker1 = new bool[4];
        LSLMarker1[0] = false;
        LSLMarker1[1] = false;
        LSLMarker1[2] = false;
        LSLMarker1[3] = false;
        //private double nominal_srate = liblsl.IRREGULAR_RATE;
        //streamInfo = new StreamInfo("Inlet", "Markers", 1, 0, LSL.channel_format_t.cf_int32, "Outlet");
        ...
        cone = GameObject.Find(cone_name);
        pointer = GameObject.Find(pointer_name);

        if (cone != null && pointer != null)
        {
            Debug.Log("started.");
            //if(cone.gameObject.activeSelf)
            //if(cone.GetComponent<MeshRenderer>().material.color != Color.black)
                set_color_cone(color_left);
            return;
        }

        if (cone == null)
        {
            Debug.Log("Failed to obtain cone.");
        }
        if(pointer = null)
        {
            Debug.Log("Failed to obtain pointer.");
        }
    ...
    // WORKSPACE OF RIVER J
    
    GameObject cone = null;
    GameObject pointer = null;
    //GameObject ans1 = null;
    //GameObject ans2 = null;
    //GameObject ans3 = null;
    //GameObject ans4 = null;
    //GameObject ans5 = null;
    //GameObject ans6 = null;

    string cone_name = "cone";
    string pointer_name = "pointer";
    

    Color color_left = Color.yellow;
    Color color_touched = Color.green;
    Color color_warning = Color.red;


    int judge_interval = 5;
    public int counter = 5;

    double touch_threshold = 3.2;

    private void set_color_cone(Color color)
    {
        if (cone != null)
        {
    ...
    void judge_distance(double distance)
    {
        Vector3 position1 = new Vector3(-3.6f, 0.0f, 0.0f);
        //Debug.Log(string.Format("Distance: {0}", distance));
        if (path_show.path_show_instace.next_index<5 && path_show.path_show_instace.next_index>0)
        //if (distance <= sinHEffect.FindObjectOfType<sinHEffect>().Dis_threshold)
        if (pointer.transform.position.x>-6.0f && pointer.transform.position.x < -4.0f && position1== cone.transform.position)
        {
            if ((HapticHand.FindObjectOfType<HapticHand>().buttonHoldDown) & (!PathFollower.shared.started) & (!PathFollower.shared.all_end))
                {//print("yesssss");
                    if (PathFollower.shared != null)
                {
                    if ((path_show.path_show_instace.next_index == 1) & (sinHEffect.FindObjectOfType<sinHEffect>().i_trial1 == 0))
                    {
                        //print("yesssss");
                        sinHEffect.FindObjectOfType<sinHEffect>().start_ref_time = System.DateTime.Now;//lida//we are in the first path of 4 paths but the first trial//used for calculating the whole spending time doing an experimnets (4 paths)
                        sinHEffect.FindObjectOfType<sinHEffect>().start_each_path_time = sinHEffect.FindObjectOfType<sinHEffect>().start_ref_time;
                        
                    }
                    else
                    {
                        sinHEffect.FindObjectOfType<sinHEffect>().start_each_path_time = System.DateTime.Now;//lida//saving the start time of each block
                        //temp_ = sinHEffect.FindObjectOfType<sinHEffect>().each_before;
                    }

                    temp_ = sinHEffect.FindObjectOfType<sinHEffect>().start_each_path_time;
                    //print("Path starting@: " + path_show.path_show_instace.next_index);//lida//current time
                    //lida//marker//here//sinHEffect.FindObjectOfType<sinHEffect>().marker.Write(path_show.path_show_instace.next_index);//lida//write(path number:1,2,3,4)
                    LSLMarker1[path_show.path_show_instace.next_index - 1] = true;
                    //int lg1;
                    //lg1 = path_show.path_show_instace.next_index;
                    //currentSample[0] = path_show.path_show_instace.next_index; //lg1;
                    //outlet.push_sample(currentSample);
                    sinHEffect.FindObjectOfType<sinHEffect>().eeg_text = "Path " + path_show.path_show_instace.next_index + ":                    @ " + temp_.ToString("HH:mm:ss.fff");//lida//current time
                    //string[] lines = { text }; File.AppendAllLines(Path.Combine(sinHEffect.FindObjectOfType<sinHEffect>().docPath, sinHEffect.FindObjectOfType<sinHEffect>().out_result), lines);
                    PathFollower.shared.started = true;
                    sinHEffect.FindObjectOfType<sinHEffect>().finishedpathFlag = 0;
                    sinHEffect.FindObjectOfType<sinHEffect>().previous_path = sinHEffect.FindObjectOfType<sinHEffect>().stylPosition;
                    sinHEffect.FindObjectOfType<sinHEffect>().clickflg = true;
    ...
    void FixedUpdate() 
	{
        ...
        if (cone == null) cone = GameObject.Find(cone_name);
        if (pointer == null) pointer = GameObject.Find(pointer_name);

        // get the distance from pointer to the cone. (only when counter exeeds the interval)
        counter++; //lida//counter = 5;
        //print("plug  " + counter);
        if (counter >= judge_interval && sinHEffect.FindObjectOfType<sinHEffect>().is_touching)//lida//judge_interval = 5;after the first frame in every 5 frames it will be true
        {
            counter = 0;
            //distance_from_pointer_to_cone = Vector3.Distance(cone.transform.position, pointer.transform.position);
            //print("distance between the cone and the pointer:" + Vector3.Distance(cone.transform.position, pointer.transform.position));
            //judge_distance(distance_from_pointer_to_cone);
            //Vector3 position1 = new Vector3(-3.6f, 0.0f, 0.0f);
            //Debug.Log(string.Format("pointer:   "+ pointer.transform.position)); //Debug.Log( pointer);
            sinHEffect.FindObjectOfType<sinHEffect>().DisPtoC = Vector3.Distance(cone.transform.position, pointer.transform.position);//(position1, pointer.transform.position); //
            //sinHEffect.FindObjectOfType<sinHEffect>().DisPtoC=
            judge_distance(sinHEffect.FindObjectOfType<sinHEffect>().DisPtoC);
        }

        //pasdfrisnt("distance between the cone and the pointer:" + sinHEffect.FindObjectOfType<sinHEffect>().DisPtoC);

        if (touchingDepth > touch_threshold)//lida//touch_threshold = 3.0;
        {
            //warningText.SetActive(true);
            //if (cone.gameObject.activeSelf)
                //if (cone.GetComponent<MeshRenderer>().material.color != Color.black)
                set_color_cone(color_warning);//lida//red
        }
        else
        {
        ...
        if (shapesEnabled)
		{
            ...
            if (shape_getTouched(configName, ref shapeID, ref depth))
            {
                if (touching == null || touching.GetInstanceID() != shapeID)
                {
                    for (int ii = 0; ii < touchableObjects.Length; ii++)
                    {
                        ...
                        {
                            // handler touched
                            sinHEffect.FindObjectOfType<sinHEffect>().is_touching = true;
                           // if (cone.GetComponent<MeshRenderer>().material.color != Color.black)
                                set_color_cone(color_touched);
                            //print("green");
            ...
            {
                if(sinHEffect.FindObjectOfType<sinHEffect>().is_touching)
                {
                    // handler leaving
                    //if (cone.GetComponent<MeshRenderer>().material.color != Color.black)
                        set_color_cone(color_left);//yellow
                }
                sinHEffect.FindObjectOfType<sinHEffect>().is_touching = false;
                touching = null;
            }
	...
	private void updateDevice()
	{
        ...
        //****************************************************************LIDA
        //print("Lida HP-UD-World" + stylusPositionWorld); //pointer position
        //print("distance-HE   " + plane1.GetDistanceToPoint(stylusPositionWorld));//Lida//printing the distance between the curved surface and the a plane in the background
	...
	private void updateManipulator()
	{
        ...
        //print("11111111body.velocity="); print(body.velocity);
	...
	public void safeUpdateManipulator()
	{
		...
		
		{
			body.position = stylusPositionWorld;
			body.rotation = stylusRotationWorld;
			body.velocity = Vector3.zero; //print("222body.velocity="); print(body.velocity);
		}
