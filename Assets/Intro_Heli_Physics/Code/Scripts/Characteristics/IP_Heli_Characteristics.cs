...
namespace IndiePixel
{
    public class IP_Heli_Characteristics : MonoBehaviour
    {
        ...
        protected virtual void HandleLift(Rigidbody rb, IP_Input_Controller input)
        {
            ...
            if (mainRotor)
            {
                Vector3 liftForce = transform.up * (Physics.gravity.magnitude + maxLiftForce) * rb.mass;//counteracting the gravity that is applied for the object
                float normalizedRPMs = mainRotor.CurrentRPMs / 500f; //500=(3000(maxR)/360)*60 -- normalizedRPMs: how fast the rotor blades are spinning -- if normalizedRPMs==1 it means that they're rotationg full power 
